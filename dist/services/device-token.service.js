...
let DeviceTokenService = DeviceTokenService_1 = class DeviceTokenService {
    ...
    async upsertDeviceToken(userId, deviceToken) {
        ...
        try {
            ...
            await pool.query(`INSERT INTO device_tokens (user_id, device_token, platform, is_active, last_used_at, created_at, updated_at)
         VALUES ($1, $2, 'ios', true, NOW(), NOW(), NOW())
         ON CONFLICT (user_id, device_token)
         DO UPDATE SET
           is_active = true,
           last_used_at = NOW(),
           updated_at = NOW()`, [userId, deviceToken.trim()]);
