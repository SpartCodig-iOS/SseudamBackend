    ...
    async getAvatarUrlOnly(userId) {
        try {
            ...
            if (storageAvatar) {
                ...
                pool.query(`UPDATE profiles SET avatar_url = $2, updated_at = NOW() WHERE id = $1`, [userId, storageAvatar]).catch(err => this.logger.warn(`[getAvatarUrlOnly] Failed to persist storage avatar for ${userId}: ${err.message}`));
