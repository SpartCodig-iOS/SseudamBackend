...
// Service
// ─────────────────────────────────────────────────────────────────────────────
let TravelSettlementService = TravelSettlementService_1 = class TravelSettlementService {
    constructor(dataSource, cacheService) {
        this.dataSource = dataSource;
        this.cacheService = cacheService;
        this.logger = new common_1.Logger(TravelSettlementService_1.name);
        this.SETTLEMENT_PREFIX = 'settlement:summary';
        this.SETTLEMENT_TTL = 60; // 1분 캐시 (실시간성 강화)
    }
    // ───────────────────────────────────────────────────────────────────────────
    // Private 헬퍼
    // ───────────────────────────────────────────────────────────────────────────
    ...
    async fetchBalances(travelId, manager) {
    ...
    // 감사 로그 기록
    // ───────────────────────────────────────────────────────────────────────────
    async writeAuditLog(manager, params) {
        try {
            await manager.query(`INSERT INTO settlement_audit_logs
         ...
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9::jsonb)`, [
            ...
            this.logger.warn(`[AuditLog] Failed to write audit log for ${params.action} on travel ${params.travelId}: ${err.message}`);
        }
    }
    // ───────────────────────────────────────────────────────────────────────────
    // Public API
    ...
    async getSettlementSummary(travelId, userId) {
        ...
        const savedSettlements = storedSettlements.map((row) => ({
            id: row.id,
        ...
        const summary = { balances, savedSettlements, recommendedSettlements };
        this.cacheService
     ...
     */
    async saveComputedSettlements(travelId, userId, options = {}) {
        await this.ensureMember(travelId, userId);
        // 멱등성 키 체크: 동일 키가 이미 처리된 경우 캐시 결과 반환
        if (options.idempotencyKey) {
            const idempotencyPrefix = 'settlement:idempotency';
            try {
                const existingResult = await this.cacheService.get(options.idempotencyKey, { prefix: idempotencyPrefix });
                if (existingResult) {
                    this.logger.debug(`[saveComputedSettlements] Idempotent replay for key=${options.idempotencyKey} travelId=${travelId}`);
        ...
        const versionRows = await this.dataSource.query(`SELECT COALESCE(MAX(version), 0) AS max_version
       FROM travel_settlements
       WHERE travel_id = $1`, [travelId]);
        const currentVersion = Number(versionRows[0]?.max_version ?? 0);
        ...
        await this.ensureTransaction(async (manager) => {
            ...
            const lockRows = await manager.query(`SELECT COALESCE(MAX(version), 0) AS max_version
         FROM travel_settlements
         WHERE travel_id = $1
         FOR UPDATE`, [travelId]);
            const lockedVersion = Number(lockRows[0]?.max_version ?? 0);
            if (lockedVersion !== currentVersion) {
                throw new common_1.ConflictException('다른 사용자가 동시에 정산을 수정했습니다. 최신 정산 내역을 확인한 후 다시 시도해주세요.');
            }
            // 기존 정산 전부 삭제 후 재삽입
            ...
            await manager.query(`INSERT INTO travel_settlements
         ...
         FROM UNNEST(
           ...
           $5::numeric[],
           $6::integer[]
         ) AS t(id, travel_id, from_member, to_member, amount, version)`, [ids, travelIds, fromMembers, toMembers, amounts, versions]);
            // 감사 로그 기록
        ...
        this.logger.log(`[saveComputedSettlements] travelId=${travelId} actor=${userId} ` +
    ...
    async markSettlementCompleted(travelId, userId, settlementId) {
        ...
        await this.ensureTransaction(async (manager) => {
            ...
            const currentRows = await manager.query(`SELECT status, version
         FROM travel_settlements
         WHERE id = $1 AND travel_id = $2
         FOR UPDATE`, [settlementId, travelId]);
            if (!currentRows[0]) {
                throw new common_1.BadRequestException('정산 내역을 찾을 수 없습니다. 계산된 결과를 저장한 뒤 완료 처리하세요.');
            }
            oldStatus = currentRows[0].status;
            const currentRowVersion = Number(currentRows[0].version ?? 1);
            newVersion = currentRowVersion + 1;
            // 이미 완료된 경우 멱등성 보장 (재처리 무시)
            if (oldStatus === 'completed') {
                this.logger.debug(`[markSettlementCompleted] settlementId=${settlementId} already completed, skipping`);
                return;
            }
            const rows = await manager.query(`UPDATE travel_settlements
         SET status       = 'completed',
             completed_at = NOW(),
             updated_at   = NOW(),
             version      = $3
         WHERE id = $1 AND travel_id = $2 AND version = $4
         RETURNING id`, [settlementId, travelId, newVersion, currentRowVersion]);
            // affected 0 → 다른 트랜잭션이 선점 수정
            if (!rows[0]) {
                throw new common_1.ConflictException('다른 사용자가 동시에 동일 정산을 수정했습니다. 잠시 후 다시 시도해주세요.');
            }
            await this.writeAuditLog(manager, {
                travelId,
                settlementId,
...
exports.TravelSettlementService = TravelSettlementService = TravelSettlementService_1 = __decorate([
