...
export class OptimizedDeleteService {
  ...
  private async performFastLocalDeletion(userId: string): Promise<string[]> {
    ...
    try {
      ...
      const profileRow = await client.query<{ name: string | null }>(
      ...
      const oauthTable = await client.query<{ name: string | null }>(
      ...
      await client.query(
        `UPDATE travel_expenses
           SET display_name = COALESCE(display_name, $2),
               payer_id = NULL,
               author_id = NULL
         WHERE payer_id = $1 OR author_id = $1`,
        [userId, displayName],
      );
      await client.query(
        `UPDATE travel_expense_participants
           SET display_name = COALESCE(display_name, $2),
               member_id = NULL
         WHERE member_id = $1`,
        [userId, displayName],
