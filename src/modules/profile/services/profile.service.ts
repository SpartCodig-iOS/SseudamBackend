...
import { UpdateProfileInput } from '../validators/profile.validators';
import 'multer';
...
export class ProfileService {
  ...
  private readonly imageVariantsCache = new Map<string, { variants: ImageVariant[]; expiresAt: number }>();
  ...
  private getCachedImageVariants(userId: string): ImageVariant[] | null {
  ...
  private setCachedImageVariants(userId: string, variants: ImageVariant[]): void {
   ...
   */
  private cacheImageVariants(userId: string, variants: ImageVariant[]): void {
    // 메모리 캐시
  ...
  private async uploadToSupabase(userId: string, file: Express.Multer.File): Promise<string> {
    ...
    try {
      ...
      // 모든 이미지 변형 업로드 (병렬 처리)
      const uploadPromises = imageVariants.map(async (variant: ImageVariant) => {
      ...
      const originalVariant = uploadedVariants.find((variant) => variant.size === 'original');
      if (!originalVariant) {
        throw new BadRequestException('원본 이미지 업로드에 실패했습니다.');
      }

      // 캐시에 모든 변형 저장 (빠른 로딩용) - 간단한 타입으로 변환
      const variantsForCache: ImageVariant[] = uploadedVariants.map((variant) => ({
        size: variant.size,
        filename: `${userId}_${variant.size}.jpg`,
        buffer: Buffer.from(''),
      ...
      this.cacheImageVariants(userId, variantsForCache);
  ...
  async getAvatarThumbnail(userId: string): Promise<string | null> {
    try {
      ...
      if (cachedVariants) {
        ...
        if (thumbnail) return thumbnail.url ?? null;
      }

      // 2. Redis에서 변형들 조회
      try {
        const redisVariants = await this.cacheService.get<ImageVariant[]>(userId, { prefix: this.AVATAR_VARIANTS_PREFIX });
        if (redisVariants) {
  ...
  async getAvatarMedium(userId: string): Promise<string | null> {
    try {
      ...
      if (cachedVariants) {
        ...
        if (medium) return medium.url ?? null;
      }
