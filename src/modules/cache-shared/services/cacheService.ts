...
export class CacheService {
   ...
   */
  async getTtl(key: string, config: CacheConfig = {}): Promise<number> {
    const redis = await this.getRedisClient();
    const cacheKey = config.prefix ? `${config.prefix}:${key}` : key;
    ...
    if (redis) {
      try {
        ...
        this.logger.warn(`Redis TTL failed (${this.stringifyError(error)})`);
      }
    }
   ...
   */
  async expire(key: string, seconds: number, config: CacheConfig = {}): Promise<void> {
    const redis = await this.getRedisClient();
    const cacheKey = config.prefix ? `${config.prefix}:${key}` : key;
    ...
    if (redis) {
      try {
        ...
        this.logger.warn(`Redis expire failed (${this.stringifyError(error)})`);
      }
    }
  ...
  /**
   * Tag-based cache invalidation (alias for invalidateByTags for single tag)
   */
