...
import { AuthService, AuthSessionPayload } from '../../auth/services/auth.service';
...
export class SocialAuthService {
  ...
  private primeLookupCaches(accessToken: string, cacheKey: string, result: SocialLookupResult): void {
    this.setCachedCheck(accessToken, result.registered ?? false);
  ...
  private resolveLoginType(requested: LoginType = LoginType.EMAIL, supabaseUser?: any): LoginType {
    const provider =
  ...
  async loginWithOAuthToken(
    ...
    if (loginType === 'kakao') {
      ...
      const session = await this.authService.createAuthSession(userRecord, LoginType.KAKAO);
    ...
    const userRecord = fromSupabaseUser(user);
  ...
  async checkOAuthAccount(
    ...
    if (cachedCheck) {
      ...
      return { exists: cachedCheck.registered, registered: cachedCheck.registered };
    }
  ...
  async checkKakaoAccountWithCode(
    ...
    return { exists: registered, registered };
  }
  ...
  private async performOAuthLookup(
      ...
      if (isSupabaseToken && isNotExpired) {
        try {
          ...
          if (registered.status === 'fulfilled') {
            const result = { exists: registered.value, registered: registered.value };
    ...
    if (cachedUser.status === 'fulfilled' && cachedUser.value) {
      try {
        ...
        const result = { exists: registered, registered };
