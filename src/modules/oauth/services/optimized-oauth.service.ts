...
import { SocialAuthService, OAuthTokenOptions } from '.';
import { LoginType } from '../../auth/types/auth.types';
import { AuthSessionPayload } from '../../auth/services/auth.service';
import { CacheService } from '../../cache-shared/services/cacheService';
import { SupabaseService } from '../../core/services/supabaseService';
import { createHash } from 'crypto';

// 임시 타입 정의 (SocialLookupResult만)
interface SocialLookupResult {
  exists: boolean;
  userId?: string;
  email?: string;
  profile?: any;
...
export class OptimizedOAuthService {
  ...
  async fastCheckOAuthAccount(
    ...
    try {
      ...
      const result: SocialLookupResult = { exists: Boolean(profile) };

      // 5단계: 다중 캐싱 (메모리 + Redis) - 비동기로 실행해 응답 지연 방지
      Promise.allSettled([
        this.cacheService.set(cacheKey, result, { ttl: 300 }), // Redis 5분
        // 메모리 캐시는 기존 메서드 활용
        Promise.resolve((this.socialAuthService as any).setCachedCheck?.(accessToken, result.exists))
      ]);

      const duration = Date.now() - startTime;
      // this.logger.debug(`FAST lookup completed: ${duration}ms (exists: ${result.exists})`);
