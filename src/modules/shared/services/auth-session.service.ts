...
// import { SessionService, SessionRecord } from '../auth/services/sessionService'; // 삭제됨
...
export class AuthSessionService {
  ...
  async createAuthSession(
    ...
    try {
      const sessionRecord = {
        ...
        userId: userId,
        loginType: (sessionData.metadata?.loginType as LoginType) || 'email',
        createdAt: new Date(),
  ...
  async deleteAuthSession(sessionId: string): Promise<void> {
    try {
      ...
      // 기본 세션 서비스에서도 삭제 (로그만 출력)
      this.logger.debug(`Would delete session from session service: ${sessionId}`);
  ...
  async touchSession(sessionId: string): Promise<void> {
    try {
      // 기본 세션 서비스의 touch 기능 사용 (로그만 출력)
      this.logger.debug(`Would touch session: ${sessionId}`);
    } catch (error) {
