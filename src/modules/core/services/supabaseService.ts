...
export class SupabaseService {
  ...
  async findProfilesByIds(ids: string[]) {
    ...
    const profileMap = new Map(result.rows.map((profile: any) => [profile.id, profile]));
  ...
  async findProfilesByIdsWithCache(ids: string[], cacheService?: any): Promise<any[]> {
    ...
    if (uncachedIds.length > 0) {
      ...
      // 조회한 프로필들을 캐시에 저장
      if (cacheService) {
        const cachePromises = uncachedProfiles.map((profile: any) =>
  ...
  async mirrorProfileAvatar(userId: string, sourceUrl?: string | null): Promise<string | null> {
    ...
    try {
      ...
      // 모든 이미지 변형 병렬 업로드
      const uploadPromises = imageVariants.map(async (variant: any) => {
